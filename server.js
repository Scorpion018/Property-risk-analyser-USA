...
let data = ''
// let recalldata = [
//   newMessage = 'No hazard Data Found'
// ]
...
if(req.body.compAddress){
  ...
  if(req.body.companyName){
    sql = `select * from csv2021 where company_name ILIKE '%${req.body.companyName}' OR company_name ILIKE '%${req.body.companyName}%' OR company_name ILIKE '${req.body.companyName}%'`
  }
  if(req.body.compAddress){
...
app.post('/draft', (req,res)=>{
  ...
  console.log('Data',recalldata)
    ...
    WHERE (csv2021.company_name ILIKE '%${req.body.companyName}' OR csv2021.company_name ILIKE '%${req.body.companyName}%' OR csv2021.company_name ILIKE '${req.body.companyName}%') 
    ...
    pool.query(sql, (err, results) => {
        if (err) {
          ...
          setTimeout(()=>{
            ...
            .length)
            console.log('Results ?',recalldata)
            console.log('latitude ?',results.rows[0].propertylatitude)
            console.log('longitude ?',results.rows[0].propertylongitude)
            res.render("Details", {
              path: results.rows,
              helper:helper,
              path1: recalldata,
              lati : results.rows[0].propertylatitude,
              longi : results.rows[0].propertylongitude
            });
            // let abcdEf = results.rows
            // abcdEf = [...abcdEf , recalldata]  
            // res.json(abcdEf)
          } , 5000)
